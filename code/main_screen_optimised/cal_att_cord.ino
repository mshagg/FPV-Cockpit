void cal_att_cord(){
  ...
  x0 = r0*cosa + x_center; //x-coordinate of 0 degree midpoint
  y_0 = r0*sina + y_center; //y-coordinate
  vx = level_pixel*cosa;
  vy = level_pixel*sina;

  center_degree = (pitch)/PI*180; //Convert pitch from rads to degrees
  base_vect = round(center_degree/level_angle); // Count how many attitude lines lie between x0/y_0 and the attitude indicator centre
  //0-degree attitude line coordinates
  xa1 = x0+cosb*r1;
  ya1 = y_0+sinb*r1;
  xa2 = x0+cosb*r2;
  ya2 = y_0+sinb*r2;  
